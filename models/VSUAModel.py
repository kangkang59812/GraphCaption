...
class GNN(nn.Module):
    ...
    def forward(self, obj_vecs, attr_vecs, rela_vecs, edges, rela_masks=None):
        ...
        # attr
        new_attr_vecs = self.gnn_attr(
            torch.cat([obj_vecs, attr_vecs], dim=-1)) + attr_vecs
...
class AttModel(CaptionModel):
    ...
    def _proj_vsu(self, obj_embed, attr_embed, rela_embed, att_feats):
        ...
        # 将label的embeding和att_feats级联，再加上att_feats
        obj_vecs = self.proj_obj(
            torch.cat([att_feats, obj_embed], dim=-1))+att_feats
    ...
    def _prepare_vsu_features(self, sg_data, att_feats, att_masks):
        ...
        att_masks, rela_masks = att_masks.unsqueeze(
            -1), rela_masks.unsqueeze(-1)
        # node features
    ...
    def _forward(self, sg_data, fc_feats, att_feats, seq, att_masks=None):
        core_args = self.prepare_core_args(
            sg_data, fc_feats, att_feats, att_masks)
        ...
        for t in range(self.seq_length + 1):
            ...
            if sample_max:
                ...
                if temperature == 1.0:
                    ...
                    prob_prev = torch.exp(
                        torch.div(logprobs.data, temperature))
...
class VSUACore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.lang_lstm = nn.LSTMCell(
            lang_lstm_in_dim, opt.rnn_size)  # h^1_t, \hat v
    ...
    def forward(self, xt, state, core_args):
        ...
        if 'o' in self.opt.vsua_use:
            att_obj = self.attention_obj(
                h_att, obj_feats, p_obj_feats, att_masks)
            lang_lstm_input = torch.cat([lang_lstm_input, att_obj], 1)

        if 'a' in self.opt.vsua_use:
            att_attr = self.attention_attr(
                h_att, attr_feats, p_attr_feats, att_masks)
            lang_lstm_input = torch.cat([lang_lstm_input, att_attr], 1)

        if 'r' in self.opt.vsua_use:
            att_rela = self.attention_rela(
                h_att, rela_feats, p_rela_feats, rela_masks)
