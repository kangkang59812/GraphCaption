...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        for split in self.split_ix.keys():
            self.logger.info('assigned %d images to split %s' %
                             (len(self.split_ix[split]), split))
    ...
    def get_captions(self, ix, seq_per_img):
        ...
        if ncap < seq_per_img:
            ...
            seq = self.h5_label_file['labels'][ixl: ixl +
                                               seq_per_img, :self.seq_length]
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            ...
            gts.append(
                self.h5_label_file['labels'][self.label_start_ix[ix] - 1: self.label_end_ix[ix]])
    ...
    def batch_sg(self, sg_batch, max_att_len):
        ...
        if self.geometry_relation:
            sg_data['rela_feats'] = np.zeros(
                [len(rela_batch), max_rela_len, self.opt.geometry_rela_feat_dim], dtype='float32')
        else:
            sg_data['rela_feats'] = np.zeros(
        ...
        for i in range(len(rela_batch)):
            sg_data['rela_edges'][i, :rela_batch[i]
                                  ['edges'].shape[0]] = rela_batch[i]['edges']
            sg_data['rela_feats'][i, :rela_batch[i]
                                  ['edges'].shape[0]] = rela_batch[i]['feats']
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            att_feat = np.load(os.path.join(
                self.input_att_dir, image_id + '.npz'))['feat']
            # Reshape to K x C
    ...
    def get_graph_data(self, index):
        ...
        sg_use = np.load(self.sg_data_dir + image_id +
                         '.npy', encoding="latin1")[()]
        if self.geometry_relation:
            geometry_path = os.path.join(
                self.sg_geometry_dir, image_id + '.npy')
            # dict contains keys of edges and feats
            rela = np.load(geometry_path, encoding="latin1")[()]
        else:
            ...
            if sg_use['rela_matrix'].shape[0] == 0:
                sg_use['rela_matrix'] = np.array(
                    [0, 0, self.sg_vocab['w2i']['near']], dtype=sg_use['rela_matrix'].dtype)
        ...
        obj = sg_use['obj_attr'][:, 1:1 +
                                 self.opt.num_obj_label_use]  # shape (No, ?)
        attr = sg_use['obj_attr'][:, 4:4 +
                                  self.opt.num_attr_label_use]  # shape (No, ?)
