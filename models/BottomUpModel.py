...
class BottomUpModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        for i in range(seq.size(1)):
            ...
            output = F.log_softmax(self.logit(
                self.dropout(output.squeeze(0))), dim=1)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            for t in range(2):
                if t == 0:
                    xt = self.img_embed(
                        fc_feats[k:k+1]).expand(beam_size, self.input_encoding_size)
                elif t == 1:  # input <bos>
