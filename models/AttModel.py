...
class AdaAtt_lstm(nn.Module):
    def __init__(self, opt, use_maxout=True):
        ...
        self.w2h = nn.Linear(self.input_encoding_size,
                             (4+(use_maxout == True)) * self.rnn_size)
        self.v2h = nn.Linear(
            self.rnn_size, (4+(use_maxout == True)) * self.rnn_size)

        self.i2h = nn.ModuleList([nn.Linear(
            self.rnn_size, (4+(use_maxout == True)) * self.rnn_size) for _ in range(self.num_layers - 1)])
        self.h2h = nn.ModuleList([nn.Linear(
            self.rnn_size, (4+(use_maxout == True)) * self.rnn_size) for _ in range(self.num_layers)])
    ...
    def forward(self, xt, img_fc, state):
        ...
        for L in range(self.num_layers):
            ...
            out_gate = sigmoid_chunk.narrow(
                1, self.rnn_size * 2, self.rnn_size)
            # decode the write inputs
            if not self.use_maxout:
                in_transform = torch.tanh(all_input_sums.narrow(
                    1, 3 * self.rnn_size, self.rnn_size))
            else:
                in_transform = all_input_sums.narrow(
                    1, 3 * self.rnn_size, 2 * self.rnn_size)
...
class AdaAtt_attention(nn.Module):
    ...
    def forward(self, h_out, fake_region, conv_feat, conv_feat_embed, att_masks=None):
        ...
        img_all = torch.cat(
            [fake_region.view(-1, 1, self.input_encoding_size), conv_feat], 1)
        img_all_embed = torch.cat(
            [fake_region_embed.view(-1, 1, self.input_encoding_size), conv_feat_embed], 1)
        ...
        if att_masks is not None:
            ...
            # assume one one at the first time step.
...
class AdaAttCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        atten_out = self.attention(
            h_out, p_out, att_feats, p_att_feats, att_masks)
...
class TopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.lang_lstm = nn.LSTMCell(
            opt.rnn_size * 2, opt.rnn_size)  # h^1_t, \hat v
...
class StackAttCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        h_0, state_0 = self.lstm0(torch.cat([xt, fc_feats], 1), [
                                  state[0][0:1], state[1][0:1]])
        att_res_1 = self.att1(h_0, att_feats, p_att_feats, att_masks)
        h_1, state_1 = self.lstm1(torch.cat([h_0, att_res_1], 1), [
                                  state[0][1:2], state[1][1:2]])
        att_res_2 = self.att2(h_1 + self.emb2(att_res_1),
                              att_feats, p_att_feats, att_masks)
        h_2, state_2 = self.lstm2(torch.cat([h_1, att_res_2], 1), [
...
class DenseAttCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        att_res_2 = self.att2(h_1 + self.emb2(att_res_1),
                              att_feats, p_att_feats, att_masks)
        h_2, state_2 = self.lstm2(torch.cat([self.fusion1(
            torch.cat([h_0, h_1], 1)), att_res_2], 1), [state[0][2:3], state[1][2:3]])
...
class Att2in2Core(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        att_res = self.attention(
            state[0][-1], att_feats, p_att_feats, att_masks)
...
class Att2all2Core(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        all_input_sums = self.i2h(
            xt) + self.h2h(state[0][-1]) + self.a2h(att_res)
        sigmoid_chunk = all_input_sums.narrow(1, 0, 3 * self.rnn_size)
        sigmoid_chunk = torch.sigmoid(sigmoid_chunk)
        in_gate = sigmoid_chunk.narrow(1, 0, self.rnn_size)
        forget_gate = sigmoid_chunk.narrow(1, self.rnn_size, self.rnn_size)
...
class LMModel(AttModel):
    def __init__(self, opt):
        ...
        delattr(self, 'fc_embed')
        self.fc_embed = lambda x: x.new_zeros(
            x.shape[0], self.input_encoding_size)
        self.embed = nn.Embedding(
            self.vocab_size + 1, self.input_encoding_size)
        self._core = LSTMCore(opt)
        delattr(self, 'att_embed')
