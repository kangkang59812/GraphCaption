...
def build_vocab(imgs, params):
    ...
    for img in imgs:
        ...
        for sent in img['sentences']:
            txt = sent['tokens']
            caption = [w if counts.get(
                w, 0) > count_thr else 'UNK' for w in txt]
...
def encode_captions(imgs, params, wtoi):
    ...
    N = len(imgs)
    M = sum(len(img['final_captions'])
            for img in imgs)  # total number of captions
    ...
    for i, img in enumerate(imgs):
        ...
        if 'filename' in img:
            jimg['file_path'] = os.path.join(
                img.get('filepath', ''), img['filename'])  # copy it over, might need
        if 'cocoid' in img:
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', default='/home/lkk/datasets/coco2014/dataset_coco.json',
                        help='input json file to process into hdf5')
    parser.add_argument('--output_json', default='cocotalk.json',
                        help='output json file')
    parser.add_argument('--output_h5', default='cocotalk', help='output h5 file')
